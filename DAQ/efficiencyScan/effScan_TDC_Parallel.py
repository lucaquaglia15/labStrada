...
def main():
    ...
    for i in range(int(len(constants.measTime))):
        ...
        print('Starting DCS process in parallel')
        dcsPath = '/home/pcald32/labStrada/DAQ/PTCorrForEffScan/DCS.py'
        print(type(effHV[0]),type(slots[0]),type(channels[0]),type(chName[0][0]),type(newRun),type(i))
        args_to_pass = {
            "effHV":effHV,
            "slots":slots,
            "channels":channels,
            "chName": [
                [base64.b64encode(item).decode() if isinstance(item, bytes) else item for item in sublist]
                for sublist in chName],
            "newRun":newRun,
            "hvPoint":i,
        }
        
        argsInJson = "args.json"
        with open(argsInJson,"w") as json_file:
            json.dump(args_to_pass, json_file)

        #DCSProcess = subprocess.Popen(["python3",dcsPath,effHV,slots,channels,chName,newRun,i], stdout = subprocess.PIPE, stderr = subprocess.PIPE)
        DCSProcess = subprocess.Popen(["python3",dcsPath,argsInJson])
        ...
        while waitingIRQ:
            ...
            if (contaMille + VMEbridge.readRegister(handle,0x1D))>=int(hex(trigNum[i]),16): #For noise scans with > 4096 triggers
                ...
                #Stop the DCS process
                DCSProcess.terminate()
                DCSProcess.wait()
